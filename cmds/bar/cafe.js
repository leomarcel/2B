...
module.exports.run = async (bot, message, args, connection) => {
    ...
    if (!target) return message.channel.send(message.author + " Mentionner quelqu'un à qui acheter un Café | `!Café <user>` :coffee:");
    ...
    if (target == message.author) {
        ...
        message.channel.send(new RichEmbed().setDescription("**Café ! :coffee:**\n" + message.author + " s'est acheter pour **" + prix + " xp** un Café ! :coffee:").setColor(1752220));
    } else {
        connection.query(`SELECT * FROM user WHERE id = '${message.author.id}'`, (err, rows) => {
            if(err) return console.log(err); //arrête le programme en cas d'erreur
        ...
        message.channel.send(new RichEmbed().setDescription("**Café ! :coffee:**\n" + message.author + " a acheter pour **" + prix + " xp** à " + target + " un Café ! :coffee:").setColor(1752220));
    }
}
