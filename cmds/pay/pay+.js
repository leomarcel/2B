...
module.exports.run = async (bot, message, args, connection) => {
	...
	if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(embedauthor("nocommand", "red", "Vous n'avez pas la permission de donner ou de retirer de l'argent à autrui :p"));
	...
	if(!add) return message.channel.send(embedauthor("command", "red","Veuillez donner une somme à donner ou à retirer"));
	const timeout = 1000;
	...
	connection.query(`SELECT * FROM user WHERE id = '${target.id}'`, (err, rows) => {
		...
		setTimeout(function(){
			msg.delete();
			message.channel.send(embed("nocommand","green", ":white_check_mark: Vous avez obtenu **¥" + add + "** \n Vous avez maintenant ¥**" + bcoins + "** ! :moneybag:"));
		}, timeout);
	});
}
